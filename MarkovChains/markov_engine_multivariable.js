/*
Markov Chain of 1st order with transition table for m variables
...
//default variables
var nthorder_wrapper  = {
  order = 1;
  position = {}; //positions to which the order is being applied
  value = {}; // values to be added to the chain
}
...
function multivariable_chain(arguments){
  if (previous.length == order) {
    ...
    var multivariable_array = arrayfromargs(arguments);
    var i, currentkey;
    var keys = previous_str.split('/');
    i = previous_str.length
    while(i >= 0) 
    {
      keys.shift();
      post("keys =", keys, "\n");
      update_corrente(chain['markov'], keys);
      i--;
    }    
    previous.shift(); 
    previous.push(current);

    post("chain['markov']['a']", chain['markov']['a']['quantity'], "\n");
    post("chain['markov']['b']", chain['markov']['b']['quantity'], "\n");
    post("chain['markov']['a']['probability']", chain['markov']['a']['probability'](), "\n");
    post("chain['markov']['b']['probability']", chain['markov']['b']['probability'](), "\n");
  } else
    previous.push(current)
}

function update_corrente(corrente, chaves, value) {
    var current_key = chaves.shift();

    if (!corrente[current_key]) {
      corrente[current_key] = {}; //add key
      corrente[current_key]['quantity'] = 1; //add 1 to quantity
      corrente['sum'] = 1; //
      var sum_pointer = function() { return corrente['sum'] };  //pointer to previous_str sum
      var quantity_pointer = function() { return corrente[current_key]['quantity']}; //pointer to current quantity
      corrente[current_key]['probability'] = function() { return quantity_pointer()/sum_pointer();};
    } else {
	  corrente[current_key]['quantity'] += 1; 		  
	}
	
    corrente['sum'] += 1; //increase the sum of previous branch

    if(chaves.length >= 1) {	
	  update_corrente(corrente[current_key], chaves, value)
    }
}
