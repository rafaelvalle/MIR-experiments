...
function multivariable_chain(){
  var multivariable_array = arrayfromargs(arguments);
  var i, currentkey;

  for(i=0; i < multivariable_array.length; i+=2) 
  {
    var keys = multivariable_array[i].split('/');
    keys.shift();
    var value = multivariable_array[i+1];
    post("keys =", keys, "\n");
	post("value =", value, "\n");
    update_corrente(chain, keys, value)};    
	post("chain['amajor']['a']['a']['duration']", chain['amajor']['a']['a']['duration'], "\n");
	post("chain['amajor']['a']['b']['duration']", chain['amajor']['a']['b']['duration'], "\n");
}

function update_corrente(corrente, chaves, value) {
  if(chaves.length == 1) {
	if (corrente[chaves[0]])
	  corrente[chaves[0]].push(value)
	else
	  corrente[chaves[0]] = value;
  } else {
